	...
	const logger = () => {
		...
		function paintFrame( id ) {
			...
			if ( logElement.innerHTML.length > 0x7FFF ) {
				...
				const newLinePos = logElement.innerHTML.indexOf( '\n', charLimit.trim );
				logElement.innerHTML = logElement.innerHTML.substring(
					-1 !== newLinePos ? newLinePos : charLimit.trim
				);
		...
		return {
			 ...
			 */
			copy: logger => {
				const logElement = document.getElementById( logger?.id || logger );

				if ( navigator.clipboard ) {
					return navigator.clipboard.writeText( logElement.innerText );
				} else {
					const text = logElement.childNodes[0];
