...
window.tsfemForm = {
	...
	prepareCollapseValidity: function( event ) {
		 ...
		 * Does initial check.
		 *
		 * @param {HTMLelement} button The label wrap checkbox button.
	...
	prepareCustomChecks: function( checkValidityCb ) {
		...
		const customChecks = function( event, element ) {
			...
			$el.one( 'tsfemForm.custom.checkValidity', checkValidityCb );
		...
		const prepareCustomChecks = function() {
			...
			$window.off( 'tsfemForm.iterationLoad.custom.validation' );
			$window.off( 'tsfemForm.iterationFail.custom.validation' );
			$window.off( 'tsfemForm.iterationComplete.custom.validation' );
			//= Event resetter.
			$window.on( 'tsfemForm.iterationLoad.custom.validation', removeCustomChecks );
			$window.on( 'tsfemForm.iterationFail.custom.validation', addCustomChecks );
			$window.on( 'tsfemForm.iterationComplete.custom.validation', addCustomChecks );
		}
		prepareCustomChecks();
	},
	...
	prepareDeiterationValidityChecks: function( checkValidityCb ) {
		...
		const disableAndValidate = function( event, target, $toRemove ) {
			...
			$items.on( 'tsfemForm.custom.deiterate.checkValidity', checkValidityCb );
			$items.each( function() { jQuery( this ).trigger( 'tsfemForm.custom.deiterate.checkValidity' ); } );
			$items.off( 'tsfemForm.custom.deiterate.checkValidity' );
		}
		/**
		...
		const prepareDeiterationChecks = function() {
			...
			$window.off( 'tsfemForm.deiterationLoad.custom.validation' );
			$window.on( 'tsfemForm.deiterationLoad.custom.validation', disableAndValidate );
		}
