...
window[ 'tsfemForm' ] = {
	...
	prepareCollapseItems: function() {
		...
		if ( $buttons ) {
			$buttons.off( 'change', prepareItems );
			$buttons.on( 'change', prepareItems );
		}
	},
	...
	prepareCollapseValidity: function( event ) {
		 ...
		 * @param {number} i The addition (or subtraction).
		 */
		const countErrors = function( $headers, i ) {

			let $header,
				newCount,
				errorClass = 'tsfem-form-collapse-header-error';

			$headers.each( function () {
				$header = jQuery( this );
				newCount = ( +$header.data( 'tsfemErrorCount' ) || 0 ) + i;

				$header.data( 'tsfemErrorCount', newCount );
				if ( newCount ) {
					$header.addClass( errorClass );
				} else {
					$header.removeClass( errorClass );
				}
			} );
		}
		/**
		 ...
		 * @param {event} event The button activation event.
		 */
		const prepareCheck = function( event ) {
			...
			if ( event.target.checked ) {
				$items.off( 'change', checkValidity );
			} else {
				$items.on( 'change', checkValidity );
			}
		}
		prepareCheck( event );
	},
	 ...
	 * Also prevents focus errors on form submission while elements are hidden.
	 *
	 * @since 1.3.0
	 * @access private
	 *
	...
	adjustSubmit: function() {
		...
		$forms.each( function() {
			jQuery( document.querySelector( '[form="' + this.id + '"]' ) ).attr( 'onclick', 'tsfemForm.saveInput( event )' );
		} );
	},
