...
window[ 'tsfemForm' ] = {
	...
	setupIterations: function() {
		...
		let $items = jQuery( '.tsfem-form-iterator-selector-wrap input' ),
			itBuffer = 0, itTimeout = 3000,
			vBar, vBarS, vBarWidth = 0, vBuffer = 0;
		...
		if ( $items.length ) {
			let fIt = ( function( e ) {
				...
				vBarStop();
				vBuffer = setInterval( vBarGo, itTimeout / 100 );
				...
				itBuffer = setTimeout( function() {
					...
					let outputWrapper = document.getElementById( e.target.id + '-wrapper' ),
						loader = document.createElement( 'div' );

				//	console.log( $item );
				//	console.log( outputWrapper );

					loader.setAttribute( 'class', 'tsfem-flex-status-loading tsfem-flex tsfem-flex-center' );
