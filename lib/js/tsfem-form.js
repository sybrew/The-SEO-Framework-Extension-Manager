...
window.tsfemForm = {
	...
	prepareCollapseValidity: function( event ) {
		...
		const checkValidity = function( event ) {
			...
			let inputIsValid = event.target.checkValidity() || event.target.disabled;
		...
		const customChecks = function( event ) {
			...
			$cItem.one( 'change.tsfemForm.custom.checkValidity', checkValidity );
			$cItem.trigger( 'change.tsfemForm.custom.checkValidity' );
		}
	...
	setupTypeListener: function() {
		...
		const find = function() {
			...
			$fields.each( function( index, element ) {
				...
				$input.data( 'typeInitTested' ) || $input.trigger( 'change.tsfemForm.typeListener' );
			} );
		}
		find();
		window.addEventListener( 'tsfemForm_iterate_complete', find );
	...
	/*		'done' : function() {
		...
		//	jQuery( target ).on( 'change', element, function( event ) {
			...
			jQuery( target ).on( 'tsfemForm.typeIsSet', function( event ) {
		//	jQuery( target ).on( 'change.' + element.id, element, function( event ) {
		...
		const init = function() {
			...
			$fields.each( function( index, element ) {
				element.dataset.showIfInit || showIfInit( element );
			} );
		}
		init();
