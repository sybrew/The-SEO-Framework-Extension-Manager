...
window[ 'tsfemForm' ] = {
	...
	setupGeo: function() {
		...
		let selectAddress = function( target, data ) {
			...
			for ( let i in data ) {
				...
				for ( let _i in components ) {
					let longname = components[ _i ]['long_name'],
						shortname = components[ _i ]['short_name'],
					...
					for ( let __i in types ) {
						...
						switch ( types[ __i ] ) {
							...
							case 'country' :
							case 'postal_code' :
								_optionValues[ i ][ types[ __i ] ] = longname;
								break loopTypes;

							case 'administrative_area_level_1' :
								_optionValues[ i ]['region'] = longname;
								break loopTypes;
		 ...
		 */
		let requestGeoData = function( event ) {

			let $wrap = jQuery( event.target ).closest( '[data-geo-api-component="action"]' ),
				$fields = getFields( $wrap ),
				_data = {};

			event.target.classList.add( 'tsfem-button-disabled', 'tsfem-button-loading' );
			...
			let completeData = {
				...
				'callbacks' : {
					...
					'failure' : function() {
						event.target.classList.remove( 'tsfem-button-disabled', 'tsfem-button-loading' );
			...
			let $_ajax = tsfemForm.getGeoData( _data, event.target.dataset.formId, completeData );
		}
		 ...
		 */
		let loadButton = function( $wrap, formId ) {
		...
		let validateField = function( $wrap, target ) {
			...
			if ( type.indexOf( 'route' ) >= 0 ) {
				...
				if ( valid < 0 ) {
					let _val = getSibblings( $wrap, target ).filter( '[data-geo-api-component="lng"]' ).first().val();
					valid = _val.replace( /[^\d\.]/, '' ) ? 1 : 0;
				}
			} else if ( type.indexOf( 'lng' ) >= 0  ) {
				valid = value.replace( /[^\d\.]/, '' ) ? -1 : 0;
				if ( valid < 0 ) {
		 ...
		 */
		let addButtonIfValid = function( event ) {
			...
			tBuffer = setTimeout( function() {
				...
				if ( $wrap.data( 'geo-api-has-button' ) ) {
					...
					//= Test only self.
					validateField( $wrap, event.target ) && loadButton( $wrap, event.target.form.id );
				}
			}, tTimeout );
		}
	...
	getGeoData: function( input, formId, completeData ) {
		...
		return jQuery.ajax( {
			...
			if ( ! data || ! type ) {
				...
				let geoData = JSON.parse( data.geodata );

				if ( 'status' in geoData ) {
					// TODO
				...
				if ( 'results' in geoData ) {
					// TODO error handling.
					output = geoData.results;
				}

				completeData.callbacks.success( completeData.target, output );
			}
		} ).fail( function( jqXHR, textStatus, errorThrown ) {
			//= Set AJAX response for wrapper.
			loaderText = tsfem.getAjaxError( jqXHR, textStatus, errorThrown );
			tsfem.setTopNotice( 1071200 );
			tsfem.setTopNotice( -1, errorThrown );
	...
	disableSubmit: function( form, loading = 1 ) {
		...
		let button = document.querySelector( '[form="' + form.id + '"]' );

		if ( loading ) {
			button.classList.add( 'tsfem-button-disabled', 'tsfem-button-loading' );
		} else {
			button.classList.add( 'tsfem-button-disabled' );
		}
		button.disabled = true;
	},
