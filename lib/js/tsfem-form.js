...
window.tsfemForm = {
	 ...
	 *
	 * @function
	 * @param {HTMLelement} element The element with data
	...
	setupGeo: function() {
		...
		const fillAddress = function( target, data ) {
			...
			$fields.each( function( index, element ) {
				...
				components = 'object' === typeof components && components || { components };
	...
	prepareCollapseItems: function() {
		...
		$buttons.off( 'change.tsfemForm.prepareItems', prepareItems );
	...
	prepareCollapseValidity: function( event ) {
		...
		const checkValidity = function( event ) {
			...
			let inputIsValid = event.target.checkValidity();

			/* Neglegible performance effect, without the added benefit of indication.
		 ...
		 * Does initial check.
		 *
		 * @param {!jQuery} $button The label wrap button.
		 * @param {!jQuery} $items The label wrap items.
		 */
		const doFirstCheck = function( $button, $items ) {

			let didCheck = $button.data( 'tsfem-did-initial-validation' ) || 0;

			if ( ! didCheck ) {
				//= Do initial run with fake event.
				let e = jQuery.Event( 'tsfem-event' );
				$items.on( 'tsfem-event.checkValidity', checkValidity );
				$items.each( function() { jQuery( this ).trigger( 'tsfem-event' ); } );
				$items.off( 'tsfem-event.checkValidity' );

				//= Register run.
				$button.data( 'tsfem-did-initial-validation', 1 );
			}
		}
		/**
		...
		const prepareChecks = function( event ) {
			...
			let $button = jQuery( event.target ),
				$items = $button.siblings( '.tsfem-form-collapse-content' ).find( 'input, select, textarea' ).not( '.tsfem-form-collapse-checkbox' );
			...
			if ( ! event.target.checked ) {
				...
				doFirstCheck( $button, $items );
			}
		}
		prepareChecks( event );

		/**
		 * Adds listener for custom validation checks.
		 * When invoked, it will directly check for the event's target validity.
		 *
		 * Once.
		 *
		 * @param {event} event The item change event.
		 * @return {undefined} Early if item details aren't specified.
		 */
		const customChecks = function( event ) {

			if ( ! ( 'item' in event.detail ) )
				return;

			let $cItem = jQuery( event.detail.item );

			$cItem.one( 'change', checkValidity );
			$cItem.trigger( 'change' );
		}
		const prepareCustomChecks = function() {
			jQuery( window ).off( 'tsfemForm_add_custom_validation' );
			jQuery( window ).on( 'tsfemForm_add_custom_validation', customChecks );
		}
		prepareCustomChecks();
	},
	...
	triggerCustomValidation: function( item ) {
		if ( item instanceof HTMLElement ) {
			...
			window.dispatchEvent( new CustomEvent( 'tsfemForm_add_custom_validation', detail ) );
		}
	},
	...
	setupSpecialRequired: function() {
		...
		const find = function() {
			...
			$fields.each( function( index, element ) {
				$box = jQuery( element );
				countVal = $box.data( 'required-check-count' );

				if ( countVal === void 0 ) {
					$items = $box.find( 'input' );
					checkedCount = $box.find( 'input:checked' ).length;

					$items.off( 'change.tsfemForm.testChecked' );
					$items.on( 'change.tsfemForm.testChecked', testChecked );
		...
		find();
		window.addEventListener( 'tsfemForm_iterate_complete', find );
	},
	...
	setupTypeListener: function() {
		...
		const find = function() {
			...
			$fields.each( function( index, element ) {
				...
				$input.off( 'change.tsfemForm.typeListener' );
				$input.on( 'change.tsfemForm.typeListener', setType );

				$input.data( 'typeInitTested' ) || $input.trigger( 'change' );
			} );
		}
		find();
		window.addEventListener( 'tsfemForm_iterate_complete', find );
	...
	setupShowIfListener: function() {
		...
		// var_dump() TODO change this into a class name with keyframes instead.
		// It's too buggy.
		let queueName = 'tsfem.showIf',
			slideOps = {
			'duration' : 250,
			'easing' : 'linear',
			'queue' : 'tsfem.showIf',
			'done' : function() {
				...
				$this.removeAttr( 'style' ).css( 'display', display );
			}
		};
		...
		const triggerHide = function( element, trigger ) {
			...
			if ( $el.is( things ) ) {
				...
				$el.closest( '.tsfem-form-setting' ).slideUp( slideOps ).dequeue( queueName );
			} else {
				//= Hide then write.
				$el.find( things ).prop( 'disabled', true ).each( function( index, _element ) {
					countDisabled( _element, 1 );
				} );
				$el.slideUp( slideOps ).dequeue( queueName );
			}
		}
		const triggerShow = function( element, trigger ) {
			...
			if ( $el.is( 'input, select, textarea' ) ) {
				...
				if ( countDisabled( element, -1 ) < 1 ) {
					element.disabled = false;
					$el.closest( '.tsfem-form-setting' ).slideDown( slideOps ).dequeue( queueName );
				}
			} else {
				//= Write then show.
				...
				$el.slideDown( slideOps ).dequeue( queueName );
			}
		}
		const attachHandler = function( element, target, value ) {
			//jQuery( target ).on( 'change.' + element.id, element, function() {} );
			jQuery( target ).on( 'change', element, function( event ) {
		...
		const showIf = function( element ) {

			element.dataset.showIfInit = 1;
		...
		const init = function() {
			...
			$fields.each( function( index, element ) {
				element.dataset.showIfInit || showIf( element );
			} );
		}
		init();
		window.addEventListener( 'tsfemForm_iterate_complete', init );
	},
	 ...
	 *
	 * @function
	 * @param {HTMLelement} form The form to enable submit from.
	 * @return {boolean} false
	 */
	 ...
	 *
	 * @function
	 * @param {HTMLelement} form The form to disable submit from.
	 * @param {boolean} loading Whether to hint it's disabled.
	 * @return {boolean} false
	 */
