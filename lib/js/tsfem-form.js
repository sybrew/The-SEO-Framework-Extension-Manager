...
window[ 'tsfemForm' ] = {
	...
	setupGeo: function() {
		 ...
		 * @return {undefined}
		 */
		let fillAddress = function( target, data ) {
		 ...
		 * @return {jQuery.element} The sibblings
		 */
		 ...
		 * @param {jQuery.element} $wrap The geodata parent button wrapper.
		 * @param {event.target} target
		 * @return {number} Whether the field is valid. Eiter 1 or 0.
		 */
		let validateField = function( $wrap, target ) {
			...
			if ( type.indexOf( 'route' ) >= 0 ) {
				...
				valid = /^((([0-9\/-]+([\/-0-9A-Z]+)?(\s|(,\s)))([\u00a1-\uffffa-zA-Z\s]|[0-9_/-])+))|(([\u00a1-\uffffa-zA-Z\s]|[0-9_/-])+)((\s|(,\s))([0-9\/-]+([\/-0-9A-Z]+)?))$/.test( value ) ? 1 : 0;
