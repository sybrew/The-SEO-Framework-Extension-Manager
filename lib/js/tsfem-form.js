...
window.tsfemForm = {
	...
	setupIterations: function() {
		...
		window.addEventListener( 'tsfemForm_iterationFail', undoInput );

		//= Loader resetter.
		window.addEventListener( 'tsfemForm_iterationLoad', resetLoaders );
		window.addEventListener( 'tsfemForm_iterationFail', resetLoaders );
		//= Event resetter.
		window.addEventListener( 'tsfemForm_iterationComplete', rebuildEvents );
		window.addEventListener( 'tsfemForm_iterationFail', rebuildEvents );
	...
	unloadIterations: function( target ) {
		...
		window.dispatchEvent( new Event( 'tsfemForm_iterationLoad' ) );
		jQuery( outputWrapper ).children( '.tsfem-form-collapse' ).slice( target.value ).remove();
		window.dispatchEvent( new Event( 'tsfemForm_iterationComplete' ) );
	...
	loadIterations: function( target ) {
		...
		jQuery.ajax( {
			...
			if ( contentType.indexOf( 'json' ) > -1 ) {
				...
				window.dispatchEvent( new Event( 'tsfemForm_iterationFail' ) );
			} else {
	...
	prepareCollapseValidity: function( event ) {
		 ...
		 * Once.
		 *
		 * NOTE: Does not run for closed collapse fields, because RC bugs will occur
		 * during iterations.
		 *
		 * @param {event} event The window trigger event.
		 * @param {HTMLElement} element The element to check. This element is cloned
		...
		const customChecks = function( event, element ) {
			...
			$el.one( 'tsfemForm.custom.checkValidity', checkValidity );
			$el.trigger( 'tsfemForm.custom.checkValidity' );
		}
		/**
		...
		const addCustomChecks = function() {
			jQuery( window ).on( 'tsfemForm.custom.validation', customChecks );
		}
		/**
		...
		const removeCustomChecks = function() {
			jQuery( window ).off( 'tsfemForm.custom.validation' );
		}
		/**
		...
		const prepareCustomChecks = function() {
			...
			$window.off( 'tsfemForm_iterationLoad.custom.validation' );
			$window.off( 'tsfemForm_iterationFail.custom.validation' );
			$window.off( 'tsfemForm_iterationComplete.custom.validation' );
			//= Event resetter.
			$window.on( 'tsfemForm_iterationLoad.custom.validation', removeCustomChecks );
			$window.on( 'tsfemForm_iterationFail.custom.validation', addCustomChecks );
			$window.on( 'tsfemForm_iterationComplete.custom.validation', addCustomChecks );
		}
		prepareCustomChecks();
	},
	...
	triggerCustomValidation: function( item ) {
		if ( item instanceof HTMLElement )
			jQuery( window ).trigger( 'tsfemForm.custom.validation', [ item ] );
	},
	...
	setupSpecialRequired: function() {
		...
		find();
		window.addEventListener( 'tsfemForm_iterationComplete', find );
	},
	...
	setupShowIfListener: function() {
		 ...
		 *
		 * @param {HTMLelement} element The element that might be hidden or shown.
		...
		init();
		window.addEventListener( 'tsfemForm_iterationComplete', init );
	},
