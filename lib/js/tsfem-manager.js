...
window.tsfem_manager = function () {
	 ...
	 * @sinde 2.7.0 No longer uses jQuery.
	 * @access private
	 *
	 * @function
	 * @param {Event} event
	...
	const _updateExtension = event => {
		...
		wp.ajax.send(
			...
			// If a notice is appended, we are facing an intended error.
			if ( jqXHR.notice?.code ) {
				tsfem_ui.setTopNotice( jqXHR.notice?.code || 1071100 );
			} else {
				// Prepare Ajax response for wrapper.
				loaderText = tsfem.getAjaxError( jqXHR, textStatus, errorThrown );

				// Try to set top notices, regardless. 1071100 notifies that there's an error saving.
				tsfem_ui.setTopNotice( jqXHR.notice?.code || 1071100 );
	...
	const _updateExtensionDescFooter = ( actionSlug, actionCase ) => {
		...
		wp.ajax.send(
			...
			// Nothing truly important happens here about which the user needs to be aware. Ignore.
			if ( ! data || ! type ) return;

			const footer    = document.querySelector( `#${actionSlug}-extension-entry .tsfem-extension-description-footer` ),
				  direction = 'activate' === actionCase ? 'up' : 'down';

			footer.classList.add( `tsfem-flip-hide-${direction}` );
			...
			setTimeout( () => {
				footer.classList.add( `tsfem-flip-show-${direction}` );
			}, 500 );
			setTimeout( () => {
				footer.classList.remove( `tsfem-flip-hide-${direction}`, `tsfem-flip-show-${direction}` );
			}, 750 );
		} ).fail( ( jqXHR, textStatus, errorThrown ) => {
	...
	const _doReady = () => {
		...
		document.querySelectorAll( '.tsfem-button-extension-activate, .tsfem-button-extension-deactivate' )
			.forEach( el => { el.addEventListener( 'click', _updateExtension ) } );
	}
