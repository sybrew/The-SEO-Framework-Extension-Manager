...
window.tsfem = {
	 ...
	 * @since 1.5.0 : No longer requires data-desc to be set, but can now also take titles.
	 * @access private
	 *
	 * @function
	 * @param {jQuery.event} event jQuery event
	 * @return {(boolean|undefined)} false If event is propagated or no title is present.
	 */
	enterDescHover: function( event ) {
		...
		if ( ! desc ) {
			...
			if ( title ) {
				desc = title;
				event.target.dataset.desc = title;
			} else {
				return false;
			}
		}
