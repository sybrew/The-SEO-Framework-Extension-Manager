...
window[ 'tsfem' ] = {
	...
	dialog: function( options ) {
		...
		if ( text ) {
			...
			modal.dialog.appendChild( modal.textWrap );
		}
		...
		if ( select ) {
			...
			modal.dialog.appendChild( modal.selectWrap );
		}
		...
		if ( confirm || cancel ) {
			...
			if ( confirm ) {
				...
				modal.confirmButton.className = 'tsfem-modal-confirm tsfem-button tsfem-button-small';
				modal.confirmButton.innerHTML = confirm;
				modal.confirmButton.addEventListener( 'click', function() {
					window.dispatchEvent( new Event( 'tsfem_modalConfirm' ) );
				} );
		...
		(function(){
			let opacity = 0,
				...
				fadeGo = function() {
					cO = ++opacity / 100;
					modal.container.style.opacity = cO;
		...
		jQuery( modal.mask ).on( 'wheel touchmove', preventDefault );

		let removeModal = function() {
			jQuery( modal.mask ).off( 'wheel touchmove', preventDefault );
