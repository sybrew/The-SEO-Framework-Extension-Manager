...
window.tsfem = {
	 ...
	 *
	 * @since 1.0.0
	 * @access private TODO make public.
	 *
	 * @function
	 * @return {undefined}
	 */
	 ...
	 * @return {undefined}
	 */
	fadeIn: function( target, ms = 250, show = true ) {

		if ( target === void 0 || ! target instanceof HTMLElement )
			return;
	 ...
	 * @return {undefined}
	 */
	fadeOut: function( target, ms = 300 ) {
		tsfem.fadeIn( target, ms, false );
	},

	/**
	 * Extends Window & jQuery by adding tsfem functions.
	 *
	 * Nevermind... this only works in debugging. To make it always work,
	 * functions' content need to be hashed, which is dreadful.
	 *
	 * @since 1.3.0
	 * @access private
	 *
	...
	prototype: function() {
		 ...
		 * Set main handler.
		 */
		let t = jQuery.fn.tsfem = function() {
			return;
		}

		/**
		 * Extends $.on by preventing memory leaks when in-the-loop methods are
		 * added to events.
		 * By adding namespaces it prevents event collision too.
		 *
		 * @return {!jQuery.on} this.on
		 */
		t.on = function() {

			if ( arguments.length > 2 )
				return jQuery( this ).on( arguments[0] + '.tsfem' + arguments[2].name, arguments[1], arguments[2] );

			return jQuery( this ).on( arguments[0] + '.tsfem' + arguments[1].name, arguments[1] );
		}

		/**
		 * Extends $.off by preventing memory leaks when in-the-loop methods are
		 * added to events.
		 *
		 * @return {!jQuery.on} this.on
		 */
		t.off = function() {
			...
			if ( arguments.length > 2 )
				return jQuery( this ).off( arguments[0] + '.tsfem' + arguments[2].name, arguments[1] );

			return jQuery( this ).off( arguments[0] + '.tsfem' + arguments[1].name );
		}
	},
