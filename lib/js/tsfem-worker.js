...
window.tsfem_worker = function() {
	...
	const setWorkerStatus = ( id, to ) => {
		if ( 'busy' === to ) {
			activeWorkers.set( id, true );
		} else {
			activeWorkers.set( id, false );
		}
	}
	 ...
	 * @return {Boolean}
	 */
	const isWorkerBusy = id => !! activeWorkers.get( id );
	 ...
	 * @return {Worker}
	 */
	const spawnWorker = ( file, id ) => workers.set( id, new Worker( file ) );
	 ...
	 */
	const getWorker = id => workers.get( id );
