...
window[ 'tsfem' ] = {
	...
	enterDescHover: function( event ) {
		...
		if ( desc !== undefined && 0 === $item.children( 'div' ).length ) {
			$item.append( '<div class="tsfem-desc-balloon">' + desc + '<div></div></div>' );

			var $balloon = $item.children( 'div.tsfem-desc-balloon' ),
				height = $balloon.height() + 28;

			$balloon.css( 'top', '-' + height + 'px' );
		}
	},
	...
	moveDescHover: function( event, pageX ) {
		...
		/* arrow is 22px wide, so 11px is half. */
		var $balloon = $item.children( 'div.tsfem-desc-balloon' ),
			$arrow = $balloon.children( 'div' ),
			pagex = pageX ? pageX : event.pageX,
			mousex = pagex - $item.offset().left - 11;

		if ( $balloon.offset() !== undefined ) {
			...
			if ( pagex > maxOffset ) {
				...
				/* 32 left absolute offset - "Padding" + mouse position + half width arrow */
				var offset = 32 - ( ( $balloon.outerWidth() - width ) / 2 ) + mousex + 11;

				$arrow.css( 'left', offset + "px" );
			}
		} else {
			$arrow.css( 'left', 32 + "px" );
		}
	},
	...
	ready: function( jQ ) {
		...
		jQ( window ).on( 'click', tsfem.resetSwitcher );
	}
};
jQuery( tsfem.ready );
