...
* **Changed:** The `mainEntityOfPage` is now the canonical URL (user adjustable), instead of the permalink (WordPress default URL).
* **Changed:** The `mainEntityOfPage` is now the canonical URL (user adjustable), instead of the permalink (WordPress default URL).
* **Note:** The structured data generated by this plugin is compatible with the new structured data of The SEO Framework. Once we drop support for older TSF versions, we'll integrate Articles into the new Schema.org graph.
