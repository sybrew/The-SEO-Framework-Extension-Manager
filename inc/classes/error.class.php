...
class Error {
	...
	public static function get_error_notice( $option ) {
		...
		$notice = static::get_error_notice_by_key( $key, true );
		...
		return static::format_error_notice( $key, $args );
	}
	 ...
	 */
	public static function get_error_notices( $options = [] ) {

		$notices = [];

		foreach ( $options as $option )
			$notices[] = static::get_error_notice( $option );
	 ...
	 * @return array|string The escaped notice. Empty string when no array key is set.
	 */
	public static function format_error_notice( $code, $args ) {
	 ...
	 */
	public static function get_error_notice_by_key( $key, $get_type = true ) {
		...
		switch ( $key ) {
			...
			case 307: // email/license mismatch
				$message = sprintf(
					...
					\tsfem()->get_my_account_link() // can never be WCM; user isn't connected yet.
				);
				$type = 'error';
				break;
			...
			case 902:
				if ( 'wcm' === \tsfem()->get_api_endpoint_type() ) {
					...
					$message = sprintf(
						...
						\tsfem()->get_my_account_link() // this can be WCM, which is troubling.
					);
				}
				$type = 'warning';
				break;
	 ...
	 * }
	 */
	public static function get_ajax_notice( $success, $code ) {

		$notice = static::get_error_notice_by_key( $code );
